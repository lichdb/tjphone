...
void linphone_call_add_to_conf(LinphoneCall *call, bool_t muted){
	...
	MSAudioEndpoint *ep;
	ms_message("linphone_call_add_to_conf(): call=%l\n",call);
...
void linphone_call_remove_from_conf(LinphoneCall *call){
	LinphoneCore *lc=call->core;
	LinphoneConference *conf=&lc->conf_ctx;
	ms_message("linphone_call_remove_from_conf(): call=%l\n",call);
...
static void add_local_endpoint(LinphoneConference *conf,LinphoneCore *lc){
	...
	entrycount++;
	ms_message("add_local_endpoint entry %d times\n",entrycount);
	ret = audio_stream_start_full(st, conf->local_dummy_profile,
				"127.0.0.1",
				65000,
				65001,
				0,
