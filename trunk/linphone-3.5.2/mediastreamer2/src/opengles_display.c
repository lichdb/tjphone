...
static void ogl_display_render_type(struct opengles_display* gldisp, enum ImageType type, bool_t clear, float vpx, float vpy, float vpw, float vph, int orientation) {
   ...
   if (screenW <= screenH) {
		ratio = (gldisp->yuv_size[type].height) / (float)(gldisp->yuv_size[type].width);
		w = screenW * vpw;
		h = w * ratio;
		if (h > screenH) {
			w *= screenH /(float) h;
		...
		ratio = gldisp->yuv_size[type].width / (float)gldisp->yuv_size[type].height;
		h = screenH * vph;
		w = h * ratio;
		if (w > screenW) {
			h *= screenW / (float)w;
	...
	if (type == REMOTE_IMAGE) {
		scale_factor = 1.0 / gldisp->zoom_factor;
		vpDim = (VP_SIZE * scale_factor) / 2;
...
#define degreesToRadians(d) (2.0 * 3.14157 * d / 360.0)
    rad = degreesToRadians(orientation);
