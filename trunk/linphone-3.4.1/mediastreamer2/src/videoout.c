...
typedef struct VideoOut
{
	...
	struct MSScalerContext *sws1;
	struct MSScalerContext *sws2;
	MSDisplay *display;
	bool_t own_display;
	bool_t ready;
	bool_t autofit;
...
static void video_out_uninit(MSFilter *f){
	...
	if (obj->sws1!=NULL){
		ff_sws_free(obj->sws1);
		obj->sws1=NULL;
	}
	if (obj->sws2!=NULL){
		ff_sws_free(obj->sws2);
		obj->sws2=NULL;
	}
...
static void video_out_process(MSFilter *f){
	...
	if (f->inputs[1]!=NULL && (inm=ms_queue_peek_last(f->inputs[1]))!=0) {
		if (obj->corner==-1){
			...
			if (ms_yuv_buf_init_from_mblk(&src,inm)==0){
				...
				if (obj->sws2==NULL){
					obj->sws2=ff_create_swscale_context(src.w,src.h,PIX_FMT_YUV420P,
											 obj->fbuf_selfview.w,obj->fbuf_selfview.h,PIX_FMT_YUV420P,
											 SWS_FAST_BILINEAR);
				}
				ms_display_lock(obj->display);
				if (ff_sws_scale(obj->sws2,src.planes,src.strides,obj->fbuf_selfview.planes, obj->fbuf_selfview.strides)<0){
			...
			if (ms_yuv_buf_init_from_mblk(&src,inm)==0){
				...
				{
					if (ff_sws_scale(obj->sws2,src.planes,src.strides, 0,
	...
	if (f->inputs[0]!=NULL && (inm=ms_queue_peek_last(f->inputs[0]))!=0) {
		...
		if (ms_yuv_buf_init_from_mblk(&src,inm)==0){
			...
			if (obj->sws1==NULL){
				obj->sws1=ff_create_swscale_context(src.w,src.h,PIX_FMT_YUV420P,
				obj->fbuf.w,obj->fbuf.h,PIX_FMT_YUV420P,
				SWS_FAST_BILINEAR);
			}
			ms_display_lock(obj->display);
			if (ff_sws_scale(obj->sws1,src.planes,src.strides, 0,
            			src.h, obj->fbuf.planes, obj->fbuf.strides)<0){
				ms_error("Error in ff_sws_scale().");
			}
