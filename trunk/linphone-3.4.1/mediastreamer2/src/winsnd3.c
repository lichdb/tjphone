...
//-----------------------------------------------------------
// 功能:将GBK字符转化为UTF-8格式的字符
// psrc:指向GBK字符的字符串指针
// pDest:指向UTF-8字符的字符串指针
// ilength: pDest的大小(不包括'\0')
// 返回值:如果出现错误，返回0
// 成功返回转换后的UTF-8的字符串的长度(不包括'\0')
//------------------------------------------------------------
int GBKToUTF8(const char* psrc,char* pDest,int ilength)
{
int len1 = 0;
wchar_t pWideChar[256];
if(!psrc)
   return 0;

len1 = MultiByteToWideChar(CP_ACP,0,psrc,-1,NULL,0);
//pWideChar = new wchar_t [len1+1];   //include '\0'
memset(pWideChar,0,256*sizeof(wchar_t));

len1 = MultiByteToWideChar(CP_ACP,0,psrc,-1,pWideChar,256);
if(!len1)
{
   //if(pWideChar)
   // delete pWideChar;
...
len1 = WideCharToMultiByte(CP_UTF8,0,pWideChar,-1,NULL,0,NULL,NULL);
if(!pDest)
{
   //if(pWideChar)
   // delete pWideChar;
...
len1 = WideCharToMultiByte(CP_UTF8,0,pWideChar,-1,pDest,ilength,NULL,NULL);
...
static void winsndcard_detect(MSSndCardManager *m){
	...
	tmp = GetSystemDefaultLCID();
	tmp = GetUserDefaultLCID();
