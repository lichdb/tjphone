...
int main(int argc, char *argv[]){
	...
	f2=ms_filter_new(MS_FILE_REC_ID);
	g729en=ms_filter_create_encoder("G729");
	ms_filter_call_method (f1, MS_FILTER_SET_SAMPLE_RATE,
		&rate);
	ms_filter_call_method (f2, MS_FILTER_SET_SAMPLE_RATE,
		&rate);
	ms_filter_call_method(g729en, MS_FILTER_SET_SAMPLE_RATE,
		&rate);
	ms_filter_call_method_noarg(f2,MS_FILE_REC_CLOSE);
	ms_filter_call_method(f2,MS_FILE_REC_OPEN,(void*)recname);
	ms_filter_call_method_noarg(f2,MS_FILE_REC_START);
	ticker=ms_ticker_new();
	ms_filter_link(f1,0,g729en,0);
	ms_filter_link(g729en,0,f2,0);
	...
	ms_filter_unlink(g729en,0,f2,0);
	ms_filter_unlink(f1,0,g729en,0);
	ms_filter_destroy(f1);
	ms_filter_destroy(g729en);
