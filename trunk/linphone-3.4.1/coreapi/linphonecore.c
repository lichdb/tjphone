...
static void proxy_update(LinphoneCore *lc){
	...
	list=ms_list_copy(lc->sip_conf.deleted_proxies);
	for(;list!=NULL;list=list->next){
		cfg = (LinphoneProxyConfig*) list->data;
...
static void analyze_buddy_lookup_results(LinphoneCore *lc, LinphoneProxyConfig *cfg){
	...
	for (elem=lc->bl_reqs;elem!=NULL;elem=ms_list_next(elem)){
		req=(BuddyLookupRequest *)elem->data;
		if (req->status==BuddyLookupDone || req->status==BuddyLookupFailure){
			if (req->results!=NULL){
				i=(BuddyInfo*)req->results->data;
		...
		if (lf->info==NULL){
			if (linphone_core_lookup_known_proxy(lc,lf->uri)==cfg){
				if (linphone_address_get_username(lf->uri)!=NULL){
					tmp=linphone_address_as_string_uri_only(lf->uri);
...
static void linphone_core_do_plugin_tasks(LinphoneCore *lc){
	...
	if (cfg){
		if (lc->bl_refresh){
			ctx=linphone_proxy_config_get_sip_setup_context(cfg);
...
LinphoneProxyConfig * linphone_core_lookup_known_proxy(LinphoneCore *lc, const LinphoneAddress *uri){
	...
	const char* addr_domain;
	for (elem=linphone_core_get_proxy_config_list(lc);elem!=NULL;elem=elem->next){
		LinphoneProxyConfig *cfg=(LinphoneProxyConfig*)elem->data;
		const char *domain=linphone_proxy_config_get_domain(cfg);
		addr_domain = linphone_address_get_domain(uri);
		if(addr_domain)
		{
			if (domain!=NULL && strcmp(domain,addr_domain)==0){
				found_cfg=cfg;
				break;
			}
		}
