...
Sal * sal_init(){
	...
	if (firsttime){
		...
		osip_trace_initialize_func (TRACE_LEVEL1,&_osip_trace_func);
		firsttime=FALSE;
	}
	eXosip_init();
	sal=ms_new0(Sal,1);
...
static void message_summary(Sal *sal, SalOp *op, char *from, eXosip_event_t *ev)
{
	osip_body_t *body=NULL;
	const char *name=NULL;
	osip_message_get_body(ev->request, 0, &body);
	if (body && body->body!=NULL){
		name=strstr(body->body,"Voice-Message: ");
		if (name==NULL) {
			ms_warning("Could not extract the Voice-Message info.");
		}else{
			char tmp[51];
			int i ;
			name+=strlen("Voice-Message: ");
			i=strlen(name);
			i = sprintf(tmp,"There are message waiting: %s",name);
			if (i>0){
				ms_message("Receiving Voice-Message  %s via SIP INOTIFY.",tmp);
				sal->callbacks.notify(op,from,tmp);
			}
		}
	}
}
...
static void call_message_new(Sal *sal, eXosip_event_t *ev){
	...
	if (ev->request){
		if (MSG_IS_INFO(ev->request)){
			...
			char *from=NULL;
			SalOp *op=find_op(sal,ev);
			//received notify£¬no op Jacky Jia
			if(op == NULL)
			{
			...
			if(h){
				/* support message-summary */
				if(osip_strcasecmp (h->hvalue, "message-summary") == 0){
					message_summary(sal, op, from, ev);
				}else {
...
static void other_request(Sal *sal, eXosip_event_t *ev){
	...
	if (strcmp(ev->request->sip_method,"MESSAGE")==0){
		...
		inc_update(sal,ev);
    }else if (strncmp(ev->request->sip_method, "NOTIFY", 6) == 0){
