...
static GtkWidget *find_conferencee_from_call(LinphoneCall *call){
	...
	GtkWidget *frame;
	if (call!=NULL){
		l=gtk_container_get_children(GTK_CONTAINER(tab));
		for(elem=l;elem!=NULL;elem=elem->next){
			frame=(GtkWidget*)elem->data;
...
void linphone_gtk_set_in_conference(LinphoneCall *call){
	...
	if (participant==NULL){
		tab=get_conference_tab(mw);
		addr=linphone_call_get_remote_address(call);
		participant=linphone_gtk_create_widget("main","callee_frame");
		viewswitch=linphone_gtk_get_widget(mw,"viewswitch");
...
void linphone_gtk_unset_from_conference(LinphoneCall *call){
	...
	box=(GtkWidget*)g_object_get_data(G_OBJECT(mw),"conference_tab");
