...
/*
 * Returns audio payload used of call.
**/
const char *linphone_call_get_audio_payload(LinphoneCall *call)
{
	if(!call) return NULL;
	if(call->audio_pt<0 || call->audio_pt> RTP_PROFILE_MAX_PAYLOADS) return NULL;
	//return call->audiostream->session->rcv.profile->payload[call->audiostream->session->rcv.pt]->mime_type;
	return call->audio_profile->payload[call->audio_pt]->mime_type;
}

/*
 * Returns video payload used of call.
**/
const char *linphone_call_get_video_payload(LinphoneCall *call)
{
	if(!call) return NULL;
	if(call->video_pt<0 || call->video_pt> RTP_PROFILE_MAX_PAYLOADS) return NULL;
	return call->video_profile->payload[call->video_pt]->mime_type;
...
void linphone_call_start_media_streams(LinphoneCall *call, bool_t all_inputs_muted, bool_t send_ringbacktone){
	const char *tool="linphone-3.4.1";//LINPHONE_VERSION;
	LinphoneCore *lc=call->core;
...
#ifdef VIDEO_ENABLED
	{
		...
		if (vstream && vstream->dir!=SalStreamInactive) {
			addr=vstream->addr[0]!='\0' ? vstream->addr : call->resultdesc->addr;
			call->video_profile=make_profile(call,call->resultdesc,vstream,&used_pt);

			call->video_pt=used_pt;
			if (used_pt!=-1){
				dir=VideoStreamSendRecv;
				cam=lc->video_conf.device;
